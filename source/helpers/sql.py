...
def batch_update(table_name, filter_column_name, update_column_name, data):
    ...
    logging.info("Updating {table_name}.{update_column_name}, "
                 "filtering by {filter_column_name}. {n} items are supposed to be updated"
                 .format(table_name=table_name, update_column_name=update_column_name,
                         filter_column_name=filter_column_name, n=len(data)))
    for t in data: 
        assert(type(t) == tuple)
        assert(len(t) == 2)
