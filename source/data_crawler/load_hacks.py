...
def load_hacks(contest):
    ...
    submission_query = Q(verdict = SUBMISSION_VERDICT.CHALLENGED) | Q(verdict = SUBMISSION_VERDICT.SKIPPED) | ~Q(testset = SUBMISSION_TESTSET.PRETESTS)
    submissions = Submission.objects.filter(submission_query, contest = contest).prefetch_related(Prefetch('author'), Prefetch('problem'))
